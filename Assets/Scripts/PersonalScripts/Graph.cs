...
public class Graph<TKey, TNodeValue> : ScriptableObject
{
    ...
    public void DisplayGraphAsMatrix()
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                row += $"{(adjacencyMatrix[i, j] == 1 ? "Si" : "No").PadRight(i)}";
            }
        }
        #endregion
    }
    public void DisplayGraphAsList()
    {
        ...
        foreach (var node in Nodes)
        {
            ...
            foreach (var entry in Nodes)
            {
                if (EqualityComparer<Node<TNodeValue>>.Default.Equals(entry.Value.Neighbors))
                {
